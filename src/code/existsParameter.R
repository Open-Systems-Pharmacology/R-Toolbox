...
existsParameter <- function(path_id = "*",  options = {}, DCI_Info = {})
{
	...
	if (length(options) == 0)
	{
		options <- list(isReference = FALSE, Type = "variable")
	} else {
		...
		{
			options<-c(options, Type = "variable")
		}
	}
	...
	id <- grep(toupper(options$Type),c("READONLY", "VARIABLE"), fixed = TRUE)
