...
getSimulationResult <- function(path_id = "*", options = {}, DCI_Info = {})
{
  ...
  else
  {
	...
	#browser()
	values <- matrix(timegrid,length(timegrid),1)
	for (j in ordering)
	{
		ma <- matrix(NA, length(timegrid),1)
		tmp <- DCI_Info$OutputTabValue[[j]]
		timeschema <- DCI_Info$OutputTabTime$Time
		for (k in 1:length(timeschema))
		{
			ma[which(values[,1] == timeschema[k])] <- tmp[k]
		}
		values <- cbind(values, ma)
	}
	colnames(values) <- c("Time", Path)
...
#     tmp <- DCI_Info$OutputTabValue[[j]]
#     k <- k +1
#     attributes(values[[k]]) <- attributes(tmp)
#   }
	return(values)
}
