...
setParameter <- function(value = numeric(0), path_id = "*", options = {}, DCI_Info = {})
{
	if (length(options) == 0)
	{
  ...
  valueIndex <- 1:length(indx)
  if (length(which(Table$ParameterType[indx] == "Table")) > 0) {
    warning("Parameters of type table have been ignored!")
    valueIndex <- valueIndex[which(Table$ParameterType[indx] != "Table")]
    indx <- indx[which(Table$ParameterType[indx] != "Table")]
  }
  
  iCol <- which(names(Table) == options$Property)
	Table[[iCol]][indx] <- value[valueIndex]
	if (toupper(options$Type) != "REFERENCE")
	{
