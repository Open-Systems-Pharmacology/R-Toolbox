...
setSpeciesInitialValue <- function(value = numeric(0), path_id = "*", options = {}, DCI_Info = {})
{
	if (length(options) == 0)
	{
		options <- list(Type = "variable", Property = "InitialValue", Index=numeric(0))
	}
	...
	if (!(options$Property %in% c("InitialValue", "ScaleFactor"))	)
	{
		stop("Invalid property provided. Use one of the following: InitialValue, ScaleFactor.")
	}
	...
	iTab <- which(names(DCI_Info$InputTab) == "VariableSpecies")
